    ...
    for itr in range(params["n_iter"]):
        ...
        if itr == 0:
            ...
            paths, envsteps_this_batch = utils.sample_trajectories(
                env, actor, params["batch_size"], MAX_VIDEO_LEN
            )
            # paths, envsteps_this_batch = utils.sample_trajectories(
            #     env, expert_policy, params["batch_size"], MAX_VIDEO_LEN
            # )  # Use this to evaluate expert_policy
        ...
        for _ in range(params["num_agent_train_steps_per_iter"]):
            ...
            rand_indices = np.random.permutation(len(replay_buffer.obs))[
                : params["train_batch_size"]
            ]
            ob_batch, ac_batch = ptu.from_numpy(
                replay_buffer.obs[rand_indices]
